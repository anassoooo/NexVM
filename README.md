...
2. The leaked legacy `service_role` key is also a JWT signed by the active legacy JWT secret. Deactivating the legacy API keys alone does not replace revoking trust in that signing key. Migrate Supabase Auth to an asymmetric signing key, rotate to it, deactivate the legacy `anon` and `service_role` API keys, then revoke the legacy signing key. Remove `SUPABASE_JWT_SECRET` from the backend environment and restart the backend so it no longer accepts HS256 tokens. Follow Supabase's migration sequence and decide whether to revoke immediately or wait for existing user sessions to expire.
